...
<div class="form-group row" ng-controller="'.$refTableName.'Controller" ng-init="'.$refTableName.'List();">
	...
	<div class="col-sm-10">
		<select class="custom-select" name="'.$instanc->Field.'" ng-model="addressInstance.'.$instanc->Field.'" ng-options="'.$refTableName.'.'.$refTableid.' as '.$refTableName.'.'.$toString.' for '.$refTableName.' in '.$refTableName.'InstanceList"'.$request.'>
